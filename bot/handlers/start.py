...
async def cmd_start(message: Message, state: FSMContext):
    ...
    user, created = await get_or_create_user(# bu yerda get_or_create_user userdan ma'lumot olibyabdi
        user_id=user.id,
        username=user.username,
        first_name=user.first_name,
        last_name=user.last_name
    )
    if created:# agar yaratilsa foydalanuvchi yangi va tilni tanlash so'raladi
        welcome_text = """
